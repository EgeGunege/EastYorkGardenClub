...
class MeetingsDetail extends Component {
    ...
    fetchMeetingDetails = async (meetingID) => {
        try {
            const response = await fetch(`https://localhost:44345/api/Meetings/${meetingID}`);
            if (!response.ok) {
        ...
        return (
            <section className="section-meeting-details">
                {<div className="container">
                    ...
                    <p className="meeting-description" dangerouslySetInnerHTML={{ __html: meetingDetails.details }}/>
                    ...
                    <div className="list-btn-box">
                        <Link className="list-btn" onClick={() => onPageChange('meetings')}>
                            List
                        </Link>
                    </div>
                    </div>
